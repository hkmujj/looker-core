 ...
 */
define(['ko', 'underscore', 'C', 'jquery', 'js/objection_check_detail'], function (ko, _, C, $, objection_check_detail) {
    var objection_check,
    //异议登记主模板
        TEM_OBJECTION_CHECK = _.template($('#tem_objection_check').html()),
    //异议登记表单
        TEM_OBJECTION_CHECK_TABLE = _.template($('#tem_objection_check_table').html()),
    ...
    objection_check = {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.resCode == C.ResCodes.SUCCESS) {
                        CONTAINER.html(TEM_OBJECTION_CHECK({}));
                        C.PageBreak.table({
                            //表格内容包裹层
                            $tablebox: $('.objection_check_table_box'),
                            //todo
                            headcontent: [{name: '异议申请编号', icon: 'iconfont sort', order_by: 'obj_Number'}, {name: '产品名称', icon: 'iconfont sort', order_by: 'product_Name'},
                                          ...
                                          {name: '异议内容', icon: 'iconfont sort', order_by: 'obj_AppealContent'}, {name: '操作', icon: '', order_by: ''}],
                            //编译后的表单
                            tem: TEM_OBJECTION_CHECK_TABLE,
                            //当前页
                            cindex: '1',
                            pageSize: '10',
                            url: URL,
        ...
        bind: function (CONTAINER,objStatus) {
            var _this = this;
            //查询按钮
            CONTAINER.on('click', '.objection_check_search', function () {
                var $this = $(this),
           ...
           //重置按钮
            CONTAINER.on('click', '.objection_check_reset', function () {
                $('.objNumber').val('');
                $('.objName').val('');
                $('.objCardNo').val('');
                $('.productName').val('');
            ...
            //详情
            CONTAINER.on('click', '.objection_check_detail', function(){
                var objId = $(this).attr('objId'),
                    data = {objId:objId};
                objection_check_detail.init(data);
            });

            //开始日期
            CONTAINER.on('focus', '#objection_check_start', function(){
                laydate({
                    elem: '#objection_check_start',
                    istime: true,
                    istoday: false,
                    max: $('#objection_check_end').val()
                });
            });
            //结束日期
            CONTAINER.on('focus', '#objection_check_end', function(){
                laydate({
                    elem: '#objection_check_end',
                    istime: true,
                    istoday: false,
                    min: $('#objection_check_start').val()
                });
            });
        }
    };
    return objection_check;
});